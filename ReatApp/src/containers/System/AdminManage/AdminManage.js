...
class AdminManage extends Component {
    ...
    componentDidMount() {
        this.props.fetchAllUserRedux("R1");
    ...
    componentDidUpdate(prevProps, prevState) {
        if (prevProps.listUsers !== this.props.listUsers) {
            this.setState({
                arrUserAdmin: this.props.listUsers,
            });
        }
    }
    // handle add new user
    ...
    createNewUser = async (data) => {
        this.props.createNewUserRedux(data, "R1");
    };

    // handle delete user
    handleDeleteUser = async (userId) => {
        await this.props.deleteOneUserRedux(userId, "R1");
    };
    ...
    doEditUser = async (user) => {
        await this.props.editOneUserRedux(user, "R1");
    };
    handlePageChange = (event, value) => {
        this.setState({ currentPage: value });
    };
    render() {
        const { arrUserAdmin, currentPage, itemPerPage } = this.state;
        ...
        const currentItems = arrUserAdmin.slice(indexOfFirstRecord, indexOfLastRecord);
        const nPages = Math.ceil(arrUserAdmin.length / itemPerPage);
        return (
            <div className="users-container">
                ...
                <div className="title text-center">
                    <FormattedMessage id="manage-user.manage-admin" />
                </div>
                <div className="mx-1">
